...
namespace WFSeaBattleGame
{
    public class Player
    {
        ...
        protected GameForm _gameForm;
        protected MapController _mapController;
        protected int _mapSize;
        protected int _markingOffset;
        public int ShipDecksAlive { get; set; } = 20;
        public MapButton[,] Map { get; set; }

        public Player(GameForm gameForm)
        {
            _gameForm = gameForm;
            _mapController = new MapController(_gameForm);
            _mapSize = MapController.MapSize;
        }
        ...
        public void GetRandomCoordinates(ref int x, ref int y)
        {
            x = _random.Next(0, _mapSize);
            y = _random.Next(0, _mapSize);
        }

        public void SpawnRandomShips()
        {
            for (int specificSizeShipSpawnAmount = _gameForm.NumberOfShipSizes;
            ...
            List<Point> coordinateCopies)
        {
            ...
            for (int specificShipPart = 0; specificShipPart < coordinateCopies.Count - GameForm.UnitSign; specificShipPart++)
            {
                for (int nextShipPart = specificShipPart + GameForm.UnitSign;
        ...
        {
            int x = 0, y = 0;
            bool canMakeShip = true;
            bool sameCoordinates = false;
            bool equalСhance = _random.Next(1, 3) == 1;
            bool isHorizontal = equalСhance;
            GetRandomCoordinates(ref x, ref y);
            List<Point> shipCoordinates = GetShipCoordinates(size, x, y, isHorizontal, randomShip: true);
            for (int i = 0; i < size; i++)
            {
                if (Map[shipCoordinates[i].X, shipCoordinates[i].Y].CanMakeShip()) continue;
                canMakeShip = false;
            }
        ...
        protected List<Point> GetShipCoordinates(int size, int x, int y, bool isHorizontal, bool randomShip)
        {
            ...
            if (size > oneSquareShipSize)
            {
                ...
                {
                    int sizeToCheck = _mapSize - size - oneSquareShipSize,
                        coordinateToCheck = isHorizontal ? x : y;
                    bool insideField = coordinateToCheck < sizeToCheck;
                    ShiftCoordinates(isHorizontal, randomShip ? insideField : _gameForm.IsSelectedShipHorizontal, ref x, ref y);
        ...
        protected Ship DeclareShip(List<Point> shipCoordinates)
        {
            ...
            List<MapButton> shipParts = new List<MapButton>();
            for (int i = 0; i < size; i++)
                shipParts.Add(Map[shipCoordinates[i].X, shipCoordinates[i].Y]);
        ...
        {
            bool isDead = ShipDecksAlive == 0;
            if (isDead) _gameForm.FinishGame();
            return isDead;
        }

        protected void MakeShipPart(int x, int y, Ship ship)
        {
            Map[x, y].IsShipPart = true;
            Map[x, y].ShipPart = ship;
            OccupyAroundShip(x, y);
        }

        private void OccupyAroundShip(int x, int y)
        {
            foreach (Point point in _mapController.GetPointsAround(x, y))
                Map[point.X, point.Y].Mark(shipPart: Map[x, y].ShipPart);
        }
    }
}